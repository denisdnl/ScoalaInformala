...
function saveDetails(){
    ...
    xhttp.open("POST","https://magicbox-robertdamoc1.c9users.io/magicbox/api/recipients/update/");
    


    xhttp.send({
      name:famList[index].name,
      ...
      id:famList[index].recipient_id
    });


  }

  q.open("GET","http://maps.google.com/maps/api/geocode/json?address="+famList[index].address);
  q.send();
    draw();
    hideEditModal();
}
...
function deleteDetails(index) {
  ...
  http.open("POST","https://magicbox-robertdamoc1.c9users.io/magicbox/api/recipients/delete");
  http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  try{
  http.send("request_id="+famList[index].recipient_id);
  }
  catch(e){
    console.log(e);
  }
...
function colete() {
	document.getElementById("nrColete").innerHTML = famList.filter((a)=>{return a.status !== "Available";}).length +"/"+ famList.length;
}
